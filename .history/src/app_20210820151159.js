...
client.on('message', (channel, tags, message, self) => {
	...
	if(message.toLowerCase() === 'hola'|| message.toLowerCase()=== 'buenas' || message.toLowerCase() === 'polir pol') {
		client.say(channel, `Hola , @${query} Bienvenute al stream, estamos felices de tenerte por aca, ojala te la pases bien. No olvides dar host DarkMode, recuerda darle host.`);
	}
});
