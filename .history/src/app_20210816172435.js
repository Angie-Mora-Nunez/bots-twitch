...
client.on('message', (channel, tags, message, self) => {
	...
	if(message.toLowerCase() === '!host') {
		...
		client.say(channel, `Recuerda que con un host podemos llegar a recomendados ,@${tags.username} Bienvenute al stream, estamos felices de tenerte por aca, ojala te la pases bien. No olvides dar host DarkMode`);
	}
});
